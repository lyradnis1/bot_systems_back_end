...
const service = async (country) => {
    ...
    //End user page progression here
    //try {
    //Open up page; set meta data and get browser execution scope
    ...
    //Analytics check first page
    const splashExecutionContext = await page.mainFrame().executionContext();
    const splashResult = await splashExecutionContext.evaluate(analyticsQA);
    ...
    let form1Result = await form1ExecutionContext.evaluate(analyticsQA);
    ...
    let form2Result = await form2ExecutionContext.evaluate(analyticsQA);
    ...
    let form3Result = await form3ExecutionContext.evaluate(analyticsQA);
    // click continue
    ...
    let form4Result = await form4ExecutionContext.evaluate(analyticsQA);
    // click continue
    ...
    let form5Result = await form5ExecutionContext.evaluate(analyticsQA);
    // click question
    ...
    const analyticsResults = thankYouResult;
